...
install() {
    bash <(curl -Ls https://github.com/Everwishing/XrayR-release/blob/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://github.com/Everwishing/XrayR-release/blob/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://github.com/Everwishing/XrayR-release/blob/main/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/everwishing/XrayR ---
  ${green}0.${plain} 修改配置
————————————————
